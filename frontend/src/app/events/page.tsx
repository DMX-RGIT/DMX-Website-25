...
export default function EventsPage() {
  ...
  // On initial load, check if there are upcoming events. If not, default to past.
  useEffect(() => {
    async function smartInit() {
      try {
        const upcoming = await api.events.list({ upcoming: "true" });
        if (upcoming.length === 0) {
          setIsUpcoming(false);
        }
      } catch {
        // Ignore — will fall through to normal fetch
      } finally {
  ...
  useEffect(() => {
    if (initialLoad) return; // Wait for smart init
    ...
    fetchEvents();
  }, [category, isUpcoming, initialLoad]);
